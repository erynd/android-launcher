...
import com.erynd.launcher.utils.ConstrainedFrameLayout
import com.erynd.launcher.utils.DownloadUtils
import com.erynd.launcher.utils.GamePackageUtils
...
class GeometryDashActivity : AppCompatActivity(), Cocos2dxHelper.Cocos2dxHelperListener, EryndUtils.CapabilityListener {
    ...
    private fun tryLoadGame() {
        ...
        EryndUtils.setContext(this)
        EryndUtils.setCapabilityListener(this)
        ...
        try {
            ...
            if (externalEryndPath.exists()) {
                ...
                if (copiedEryndPath.exists()) {
                    println("Loading Geode from ${externalEryndPath.name}")
...
//        Cocos2dxHelper.onPause()
    ...
    override fun onCapabilityAdded(capability: String): Boolean {
        return when (capability) {
            EryndUtils.CAPABILITY_EXTENDED_INPUT -> {
                mGLSurfaceView?.useKeyboardEvents = true
                true
            }
            EryndUtils.CAPABILITY_TIMESTAMP_INPUT -> {
